...
import type { ScoreDeltaExplainPayload, BUNQSYScoreComponents } from '@bunqsy/shared';
import type { ScoreLogRow } from '@bunqsy/shared';

const MIN_DELTA = 3; // Minimum point change to generate an explanation

export async function tryExplainScoreDelta(
  delta: number,
  toScore: number,
  prevRow: ScoreLogRow,
  components: BUNQSYScoreComponents,
): Promise<ScoreDeltaExplainPayload | null> {
...
async function buildReason(
  ...
  try {
    ...
    const prompt =
      `BUNQSY financial health score ${direction} by ${Math.abs(delta)} points ` +
