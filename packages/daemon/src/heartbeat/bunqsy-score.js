...
function readWeight(envKey, defaultVal) {
    const val = parseFloat(process.env[envKey] ?? '');
    return isNaN(val) ? defaultVal : val;
}
// ─── Balance ──────────────────────────────────────────────────────────────────
function computeBalanceComponent(balanceCents, salaryEur, db, accountId) {
...
// ─── Velocity ─────────────────────────────────────────────────────────────────
function computeVelocityComponent(db, accountId) {
...
function scoreGoal(goal, accounts) {
...
function computeGoalsComponent(db, accounts) {
...
function computeUpcomingComponent(db, balanceCents, profile) {
    // ── Rent proximity ──────────────────────────────────────────────────────────
    if (profile?.rent_amount != null && profile.rent_day != null && profile.rent_amount > 0) {
        ...
        const daysUntilRent = today <= rentDay
            ? rentDay - today
        ...
        if (daysUntilRent <= 7) {
            ...
            if (postRentCents < 0)
                return 0;
            if (postRentCents < rentCents * 0.5)
                return 20;
            if (postRentCents < rentCents)
                return 45;
            if (postRentCents < rentCents * 2)
                return 65;
            return 85;
        }
    ...
    const recurringRow = db
        .prepare(`SELECT COALESCE(SUM(ABS(amount)), 0) AS monthly_obligations
       FROM transactions
...
// ─── Trend ────────────────────────────────────────────────────────────────────
function computeTrend(db, currentScore) {
...
// ─── Main export ──────────────────────────────────────────────────────────────
export function computeBunqsyScore(db, snapshot) {
    ...
    const raw = components.balance * wBalance +
    ...
    const emotion = value >= 80 ? 'THRIVING' :
        value >= 60 ? 'CALM' :
            value >= 40 ? 'ALERT' :
                'ANXIOUS';
    return {
