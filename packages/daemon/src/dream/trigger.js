...
export async function triggerDream(db, wsEmit, triggerType = 'scheduled') {
    const sessionId = uuid();
    // Append-only insert — never update or delete (constitutional rule)
    db.prepare(`INSERT INTO dream_sessions (id, trigger_type, status, triggered_at)
     VALUES (?, ?, 'RUNNING', datetime('now'))`).run(sessionId, triggerType);
    ...
    const killTimeout = setTimeout(() => {
        ...
        worker.kill('SIGKILL');
        db.prepare(`UPDATE dream_sessions SET status = 'KILLED', completed_at = datetime('now') WHERE id = ?`).run(sessionId);
    }, WORKER_TIMEOUT_MS);
    worker.on('message', (raw) => {
        ...
        UPDATE dream_sessions SET
          ...
          status           = 'COMPLETED'
        WHERE id = ?
      `).run(msg.durationMs, msg.patternsUpdated, msg.patternsCreated, msg.briefingText, msg.dnaCard, JSON.stringify(msg.suggestions), sessionId);
            wsEmit({
                type: 'dream_complete',
                payload: {
                    sessionId,
        ...
        if (msg.type === 'ERROR') {
            ...
            db.prepare(`UPDATE dream_sessions SET status = 'FAILED', completed_at = datetime('now') WHERE id = ?`).run(sessionId);
        }
    });
    worker.on('error', (err) => {
        clearTimeout(killTimeout);
