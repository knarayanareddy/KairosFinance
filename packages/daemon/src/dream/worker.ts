...
async function runWorker(): Promise<void> {
  ...
  if (!sessionId) throw new Error('DREAM_SESSION_ID not set in worker env');
...
function buildTransactionSummary(txs: ReturnType<typeof getRecentTransactions>): string {
  if (txs.length === 0) return 'No transactions in the last 7 days.';
  ...
  const topTxLines = txs.slice(0, 10).map(t =>
    `  ${t.created_at.slice(0, 10)} | €${t.amount.toFixed(2)} | ${t.counterparty_name ?? 'Unknown'} | ${t.description ?? ''}`,
  ).join('\n');
...
function buildPatternsSummary(patterns: PatternRow[]): string {
  if (patterns.length === 0) return 'No patterns learned yet.';
  return patterns.slice(0, 10).map(p =>
    `- [${p.id}] ${p.name} (conf: ${(p.confidence * 100).toFixed(0)}%, hits: ${p.hit_count})`,
  ).join('\n');
}
