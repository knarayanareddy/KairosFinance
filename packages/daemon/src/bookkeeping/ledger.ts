...
import type { CategorizationResult, JournalEntry } from '@bunqsy/shared';
import { ACCOUNTS } from '@bunqsy/shared';

function debitCreditAccounts(
  amountEur: number,
...
function computeVatCents(amountCents: number, vatRate: string): number {
  if (vatRate === 'EXEMPT' || vatRate === '0') return 0;
...
export function insertJournalEntry(
  ...
  const txRow = db.prepare(`SELECT created_at FROM transactions WHERE id = ?`).get(txId) as
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    entryId, txId, date, description,
    debitAccount, creditAccount,
    amountCents, vatCents,
    result.category,
    result.isBusinessExpense ? 1 : 0,
    result.deductibilityPct,
...
export function getJournalEntriesForPeriod(
  db: Database.Database,
  startDate: string,
  endDate: string,
): JournalEntry[] {
  ...
  `).all(startDate, endDate) as Record<string, unknown>[]).map(row => ({
    id:               String(row['id']),
...
export function reconcileLedger(
  ...
  startDate: string,
  endDate: string,
): { totalDebits: number; totalCredits: number; balanced: boolean } {
  const rows = db.prepare(`
    SELECT debit_account, credit_account, amount_cents
    FROM journal_entries
    WHERE date BETWEEN ? AND ?
  `).all(startDate, endDate) as { debit_account: string; credit_account: string; amount_cents: number }[];
