...
function getPatternMatch(db, counterparty) {
...
async function llmCategorize(counterparty, description, amountEur, recentCorrections) {
    try {
...
${recentCorrections ? `Recent corrections from this user:\n${recentCorrections}` : ''}`,
        ...
        if (!catParsed.success || !vatParsed.success)
            return null;
        return {
...
// ─── Public API ───────────────────────────────────────────────────────────────
export async function categorizeTx(db, tx) {
  ...
  `).all();
    ...
    if (llmResult && llmResult.confidence >= LLM_THRESHOLD) {
        ...
        const reasons = [];
        if (llmResult.confidence < 0.80)
            reasons.push(`Low confidence: ${Math.round(llmResult.confidence * 100)}%`);
        if (amountCents > REVIEW_THRESHOLD_CENTS)
            reasons.push(`Large amount: €${amountEur.toFixed(2)}`);
        return CategorizationResultSchema.parse({
...
export async function categorizePending(db, limit = 5) {
