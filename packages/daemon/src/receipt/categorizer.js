 ...
 */
export function categorizeReceipt(db, receipt) {
    ...
    const rows = db
       ...
       LIMIT 1`)
        .all(-high, -low, from, to, -receipt.total);
...
function generateInsight(receipt) {
    if (receipt.lineItems.length === 0)
        return null;
    if (receipt.total <= 0)
        return null;
    for (const pattern of BAD_PATTERNS) {
        const matchingItems = receipt.lineItems.filter(item => pattern.keywords.some(kw => item.description.toLowerCase().includes(kw.toLowerCase())));
    ...
    const largestItem = receipt.lineItems.reduce((max, item) => item.total > max.total ? item : max, receipt.lineItems[0]);
