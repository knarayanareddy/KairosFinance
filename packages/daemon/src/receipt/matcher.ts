...
import type { ReceiptMatchResult } from '@bunqsy/shared';
...
export function matchReceiptToTransaction(
  ...
  if (!receipt) return { status: 'no_match', confidence: 0 };
...
export function matchAllUnmatchedReceipts(db: Database.Database): number {
  ...
  for (const row of unmatched) {
    ...
    if (result.status !== 'no_match') matched++;
  }
  return matched;
}
