...
function isoDate(daysBack, hour) {
    const d = new Date();
...
export async function registerDemoRoute(fastify, triggerTick, getAID) {
    ...
    fastify.post('/api/demo/fund-sandbox', async (_req, reply) => {
        ...
        const sessionRow = db
            ...
            .get();
        if (!sessionRow)
            return reply.status(500).send({ error: 'No active session' });
