...
export async function registerReceiptRoute(fastify) {
    // ── POST /api/receipt ────────────────────────────────────────────────────────
    fastify.post('/api/receipt', async (req, reply) => {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(receiptId, receipt.merchant, receipt.total, receipt.currency, receipt.date, receipt.category, JSON.stringify(receipt.lineItems), receipt.confidence, matchedTransactionId, insight);
        const result = {
            receiptId,
            receipt,
            lineItemSumValid,
    ...
    fastify.post('/api/receipt/:id/log-expense', async (req, reply) => {
        const db = getDb();
        const row = db
        ...
        if (row.logged_expense)
            return reply.send({ alreadyLogged: true });
        ...
        VALUES (?, 0, ?, ?, ?, ?, ?, ?)
      `).run(txId, -Math.abs(row.total), row.currency, row.merchant, `Receipt scan — ${row.merchant}`, row.category, row.date + 'T12:00:00Z');
