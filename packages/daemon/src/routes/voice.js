...
import { handleTriggerDream, handleSimulateFraud, handleSimulateSalary, handleFundSandbox, handleReadScore, handleReadInterventions, handleReadForecast, handleReadDreamReport, } from '../voice/actions.js';
...
function buildPlannerContext(db) {
    ...
    const jarAccounts = db
        .prepare(`SELECT jar_account_id, name, current_amount FROM goals
...
export async function registerVoiceRoute(fastify, triggerTick, getAID) {
    // ── POST /api/voice ──────────────────────────────────────────────────────────
    fastify.post('/api/voice', async (req, reply) => {
        ...
        let activeInterventionPlanId = null;
        try {
            for await (const part of req.parts()) {
                if (part.type === 'file' && part.fieldname === 'audio') {
                    ...
                    if (part.fieldname === 'pendingPlanId' && val)
                        pendingPlanId = val;
                    if (part.fieldname === 'activeInterventionId' && val)
                        activeInterventionId = val;
                    if (part.fieldname === 'activeInterventionPlanId' && val)
                        activeInterventionPlanId = val;
                }
            }
        }
        catch (err) {
        ...
        if (intent === 'deny') {
            ...
            if (activeInterventionPlanId) {
                ...
                try {
                    ...
                    if (activeInterventionId)
                        resolveIntervention(db, activeInterventionId, 'EXECUTED');
                    return reply.send({
                        kind: 'denied',
        ...
        if (intent === 'read_score')
            return reply.send({ kind: 'response', action: intent, ...handleReadScore(db), transcript });
        if (intent === 'read_interventions')
            return reply.send({ kind: 'response', action: intent, ...handleReadInterventions(db), transcript });
        if (intent === 'read_forecast')
            return reply.send({ kind: 'response', action: intent, ...handleReadForecast(db), transcript });
        if (intent === 'read_dream_report')
            return reply.send({ kind: 'response', action: intent, ...handleReadDreamReport(db), transcript });
    ...
    fastify.post('/api/voice/speak', async (req, reply) => {
        const { text } = req.body;
        if (!text?.trim()) {
