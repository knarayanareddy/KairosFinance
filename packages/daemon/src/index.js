...
function storeSession(session) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(session.installationToken, session.sessionToken, session.userId, session.keyPair.publicKeyPem, session.keyPair.privateKeyPem, session.serverPublicKey, session.expiresAt.toISOString());
}
// ─── Boot ─────────────────────────────────────────────────────────────────────
async function boot() {
    ...
    if (!session) {
        ...
        if (!apiKey)
            throw new Error('BUNQ_API_KEY is not set in environment');
    ...
    const heartbeatDeps = {
        client,
        runOracle: oracle,
        dispatchIntervention: (verdict, snapshot) => dispatchIntervention(verdict, snapshot, db),
        onScore: (score) => { setLastScore(score); wsEmit({ type: 'score_update', payload: score }); },
        onScoreDelta: (payload) => { wsEmit({ type: 'score_delta_explain', payload }); },
        onIntervention: (payload) => { wsEmit({ type: 'intervention', payload }); },
        onTickRecord: (snapshot) => {
            ...
            if (webhookPublicUrl && !webhookRegistered) {
                ...
                registerNotificationFilter(session.userId, activeAID, callbackUrl)
                    .catch((err) => console.warn('[bunqsy] Webhook registration failed (non-fatal):', err.message));
            }
        },
        onBookkeepingUpdate: (msg) => { wsEmit(msg); },
    ...
    const dreamTask = scheduleDreamMode(() => { void triggerDream(db, wsEmit, 'scheduled'); }, timezone);
