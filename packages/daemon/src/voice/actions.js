...
// ─── Dream Mode ───────────────────────────────────────────────────────────────
export async function handleTriggerDream(db, wsEmit) {
    const running = db
        .prepare(`SELECT id FROM dream_sessions WHERE status = 'RUNNING'`)
        .get();
    if (running) {
        return {
            spokenResponse: 'Dream mode is already running. I will read out the report when the analysis completes.',
        };
    }
    void triggerDream(db, wsEmit, 'manual');
    return {
        spokenResponse: 'Dream mode activated. I am now analysing your spending patterns and financial behaviours from the past 30 days. ' +
            'This takes a couple of minutes. Say "read dream report" when you are ready to hear the briefing.',
    };
}
// ─── Sandbox funding ──────────────────────────────────────────────────────────
export async function handleFundSandbox(db, accountId, triggerTick) {
    ...
    try {
        ...
        if (triggerTick)
            setTimeout(() => { void triggerTick().catch(() => { }); }, 1500);
        return {
            spokenResponse: 'Done. Five hundred euros has been requested from the bunq sandbox Sugar Daddy. ' +
...
// ─── Demo simulations ─────────────────────────────────────────────────────────
export function handleSimulateFraud(db, activeAID, triggerTick) {
    ...
    db.prepare(`INSERT INTO transactions
     ...
     VALUES (?, ?, -500.00, 'USD', 'Unknown LLC', 'CARD PURCHASE Ref: 882991X', 'shopping', ?, datetime('now'))`).run(uuid(), activeAID, txTime.toISOString().slice(0, 19).replace('T', ' '));
    if (triggerTick)
        setTimeout(() => { void triggerTick().catch(() => { }); }, 100);
    return {
        spokenResponse: 'Fraud simulation injected: five hundred US dollars to Unknown LLC at 2:14 AM. ' +
...
export function handleSimulateSalary(db, activeAID, triggerTick) {
    db.prepare(`INSERT INTO transactions
       (id, bunq_account_id, amount, currency, counterparty_name, description, category, created_at, synced_at)
     VALUES (?, ?, 3500.00, 'EUR', 'Werkgever Nederland BV', 'SALARISBETALING simulation', 'income', datetime('now'), datetime('now'))`).run(uuid(), activeAID);
    if (triggerTick)
        setTimeout(() => { void triggerTick().catch(() => { }); }, 100);
    return {
        spokenResponse: 'Salary simulation complete. Three thousand five hundred euros from Werkgever Nederland BV has been credited. ' +
            'Running financial analysis now — your BUNQSY score and savings recommendations will update shortly.',
    };
}
// ─── Read queries ─────────────────────────────────────────────────────────────
export function handleReadScore(db) {
    const row = db
        .prepare(`SELECT * FROM score_log ORDER BY logged_at DESC LIMIT 1`)
        .get();
    if (!row) {
        return {
            spokenResponse: 'No score data yet. The daemon needs to complete at least one heartbeat tick. Try again in a moment.',
        };
    }
    const label = row.score >= 75 ? 'healthy' : row.score >= 50 ? 'moderate' : 'at risk';
    return {
        spokenResponse: `Your BUNQSY financial health score is ${Math.round(row.score)} out of 100 — ${label}. ` +
...
export function handleReadInterventions(db) {
    const rows = db
        .prepare(`SELECT type, narration FROM interventions
...
export function handleReadForecast(db) {
    const row = db
        .prepare(`SELECT data FROM forecast_cache WHERE id = 1`)
        .get();
    if (!row) {
        return {
            spokenResponse: 'No forecast data available yet. The forecast is generated after the first heartbeat tick.',
        };
    }
    try {
        const points = JSON.parse(row.data);
        if (points.length === 0)
            return { spokenResponse: 'Forecast data is empty.' };
        ...
        return {
            spokenResponse: `Your 30-day forecast: ` +
...
export function handleReadDreamReport(db) {
    const row = db
        .prepare(`SELECT * FROM dream_sessions WHERE status = 'COMPLETED'
       ORDER BY completed_at DESC LIMIT 1`)
        .get();
    if (!row) {
        return {
            spokenResponse: 'No completed dream report yet. Say "trigger dream mode" to start the overnight analysis.',
        };
    }
    ...
    if (row.suggestions) {
        try {
            const list = JSON.parse(row.suggestions);
            if (list.length > 0) {
