...
function fastMatch(t) {
    if (/\b(yes|confirm|approve|go ahead|do it|sounds good|authorize|allow it)\b/.test(t))
        return 'confirm';
    if (/\b(no|cancel|deny|stop|abort|block it|never mind|reject|decline)\b/.test(t))
        return 'deny';
    if (/dream/.test(t) && /trigger|start|run|activate|mode/.test(t))
        return 'trigger_dream';
    if (/\bfraud\b/.test(t))
        return 'simulate_fraud';
    if (/salary/.test(t))
        return 'simulate_salary';
    if (/fund.*(sandbox|test)|sandbox.*(fund|money|top.?up)|sugar.?daddy|test.?fund/.test(t))
        return 'fund_sandbox';
    if (/dream/.test(t) && /report|briefing|result|latest|read/.test(t))
        return 'read_dream_report';
    if (/\bscore\b/.test(t))
        return 'read_score';
    if (/interven|alert|warn/.test(t))
        return 'read_interventions';
    if (/forecast|future balance|30.day|next month/.test(t))
...
export async function classifyIntent(transcript) {
    ...
    try {
        ...
        return valid.includes(raw) ? raw : 'financial';
    }
    catch {
        return 'financial';
    }
