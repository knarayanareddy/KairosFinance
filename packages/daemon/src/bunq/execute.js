...
export async function createExecutionPlan(steps, narratedText) {
    const plan = {
        id: uuid(),
        createdAt: new Date(),
        narratedText,
    ...
    const db = getDb();
    db.prepare(`INSERT INTO execution_plans (id, narrated_text, steps, status, created_at)
     VALUES (?, ?, ?, ?, datetime('now'))`).run(plan.id, plan.narratedText, JSON.stringify(plan.steps), plan.status);
    return plan;
}
export async function confirmPlan(planId) {
    ...
    db.prepare(`UPDATE execution_plans SET status = 'CONFIRMED', confirmed_at = datetime('now') WHERE id = ?`).run(planId);
}
export async function executePlan(planId) {
    ...
    const sessionRow = db
        .prepare(`SELECT session_token, private_key_pem, user_id
       FROM sessions ORDER BY created_at DESC LIMIT 1`)
        .get();
    if (!sessionRow)
        throw new Error('No active session found in DB');
    const baseUrl = getBunqBaseUrl();
    for (const step of steps) {
        let success = false;
        let bunqResponse = null;
        ...
        db.prepare(`INSERT INTO execution_step_results
         (id, plan_id, step_id, success, bunq_response, error_message, executed_at)
       VALUES (?, ?, ?, ?, ?, ?, datetime('now'))`).run(uuid(), planId, step.id, success ? 1 : 0, bunqResponse, errorMessage);
        if (!success) {
            throw new Error(`Step ${step.id} (${step.type}) failed: ${errorMessage}`);
        }
    }
    db.prepare(`UPDATE execution_plans SET status = 'EXECUTED', executed_at = datetime('now') WHERE id = ?`).run(planId);
}
/**
 ...
 */
export async function registerNotificationFilter(userId, accountId, callbackUrl) {
    const db = getDb();
    const sessionRow = db
        .prepare(`SELECT session_token, private_key_pem FROM sessions ORDER BY created_at DESC LIMIT 1`)
        .get();
    if (!sessionRow)
        throw new Error('No active session for webhook registration');
    const baseUrl = getBunqBaseUrl();
...
export async function cancelPlan(planId) {
    ...
    db.prepare(`UPDATE execution_plans SET status = 'CANCELLED' WHERE id = ?`).run(planId);
}
function buildStepRequest(step, userId) {
