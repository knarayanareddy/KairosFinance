...
import { BunqWebhookEventSchema } from '@bunqsy/shared';
// ─── IP allowlist ─────────────────────────────────────────────────────────────
 ...
 */
export function isAllowedOrigin(remoteIp) {
 ...
 */
export function validateWebhookRequest(rawBody, headers, session) {
    const signature = headers['x-bunq-client-signature'] ?? headers['X-Bunq-Client-Signature'];
    if (!signature)
        return false;
 ...
 */
export async function registerWebhookUrl(_client, session, publicUrl) {
