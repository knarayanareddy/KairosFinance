...
export async function runSavingsJarAgent(snapshot, db) {
    ...
    if (!apiKey)
        return { planId: null, narratedText: 'ANTHROPIC_API_KEY not configured.' };
    ...
    const steps = split.transfers.map(t => ({
        id: uuid(),
    ...
    const narratedText = `Your salary of €${salaryEur.toFixed(2)} just landed. ` +
        split.narrative +
