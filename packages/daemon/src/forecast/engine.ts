...
import type { ForecastPoint, ForecastEvent, PatternRow, ScoreLogRow } from '@bunqsy/shared';
import { getProfile } from '../memory/profile.js';
import { getAllEnabledPatterns } from '../memory/patterns.js';
import { getAverageDailySpend } from '../memory/transactions.js';
...
export function generateForecast(db: Database.Database): ForecastPoint[] {
  ...
  for (let i = 1; i <= FORECAST_DAYS; i++) {
    ...
    for (const pattern of patterns) {
      if (pattern.confidence < IMPULSE_MIN_CONF) continue;

      const trigger = classifyPatternTrigger(pattern);
      if (!trigger) continue;

      const fires =
        (trigger === 'weekend'      && (dow === 0 || dow === 6)) ||
...
export function getCachedForecast(db: Database.Database): ForecastPoint[] | null {
  const row = db
  ...
  try {
    return JSON.parse(row.data) as ForecastPoint[];
  } catch {
    return null;
  }
}

export function storeForecastCache(db: Database.Database, points: ForecastPoint[]): void {
  const now       = new Date();
...
function estimateCurrentBalance(db: Database.Database, salaryAmount: number): number {
  ...
  const row = db
    ...
    .get() as Pick<ScoreLogRow, 'balance_component'> | undefined;
...
function estimateAvgDailySpend(db: Database.Database): number {
...
type PatternTrigger = 'weekend' | 'friday' | 'monthly' | 'weekly' | 'daily' | null;

function classifyPatternTrigger(pattern: PatternRow): PatternTrigger {
  const text = `${pattern.name} ${pattern.description}`.toLowerCase();

  // Check trigger_conditions JSON for explicit keys
  let conditions: Record<string, unknown> = {};
  try {
    conditions = JSON.parse(pattern.trigger_conditions) as Record<string, unknown>;
  } catch { /* ignore */ }

  if ('day_of_week' in conditions) {
    const dow = conditions['day_of_week'];
    if (dow === 0 || dow === 6 || dow === 'weekend') return 'weekend';
    if (dow === 5 || dow === 'friday') return 'friday';
    if (dow === 1 || dow === 'monday') return 'weekly';
  }

  if ('day_of_month' in conditions) return 'monthly';
...
function estimateImpulseAmount(pattern: PatternRow, avgDailySpend: number): number {
  // Try to read amount from intervention_template
  try {
    const template = JSON.parse(pattern.intervention_template) as Record<string, unknown>;
    if (typeof template['amount'] === 'number' && template['amount'] > 0) {
      return template['amount'] as number;
    }
  } catch { /* ignore */ }
