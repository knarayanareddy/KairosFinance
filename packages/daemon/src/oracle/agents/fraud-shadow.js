...
import { getRecentTransactions, getDailySpend, getAverageDailySpend, } from '../../memory/transactions.js';
// ─── Constants ────────────────────────────────────────────────────────────────
const FALLBACK_VOTE = {
...
function buildUserPrompt(snapshot, daily, avgDaily, txLines) {
    return [
...
export async function run(snapshot, db) {
    try {
        ...
        const rawJson = JSON.parse(extractJsonText(block.text));
