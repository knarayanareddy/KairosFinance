...
import type { OracleVote, TransactionRow } from '@bunqsy/shared';
...
function recurringSpend(txs: TransactionRow[], start: string, end: string): number {
  return txs
    .filter((tx) => tx.is_recurring === 1 && tx.amount < 0 && tx.created_at >= start && tx.created_at <= end)
    .reduce((sum, tx) => sum + Math.abs(tx.amount), 0);
}

function newRecurringCounterparties(
  txs: TransactionRow[],
  currentStart: string,
  prevStart: string,
): string[] {
