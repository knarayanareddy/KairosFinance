...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function buildActivityText(snapshot, dailySpend, avgDailySpend, db) {
...
function matchesTrigger(pattern, snapshot, dailySpend) {
    let conditions;
    try {
    ...
    if (conditions.minDailySpendEur !== undefined && dailySpend < conditions.minDailySpendEur)
        return false;
    if (conditions.maxBalanceCents !== undefined && snapshot.balanceCents > conditions.maxBalanceCents)
        return false;
    if (conditions.minBalanceCents !== undefined && snapshot.balanceCents < conditions.minBalanceCents)
        return false;
    return true;
}
// ─── Agent entry point ────────────────────────────────────────────────────────
