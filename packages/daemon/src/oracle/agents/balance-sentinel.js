 ...
 */
function jointAccountRiskBoost(db, snapshot) {
    const joints = snapshot.accounts.filter((a) => a.status === 'ACTIVE' && a._wrapperType === 'MonetaryAccountJoint');
    for (const joint of joints) {
        const row = db
            .prepare(`SELECT COALESCE(SUM(ABS(amount)), 0) AS total_out
         FROM transactions
