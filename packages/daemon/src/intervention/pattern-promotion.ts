...
export async function offerPatternPromotion(
  ...
  try {
    ...
    console.error(
      '[pattern-promotion] Claude call failed:',
  ...
  if (!result.success || !result.data.isPattern) return;
  ...
  try {
    ...
    console.error(
      '[pattern-promotion] Insert failed:',
      err instanceof Error ? err.message : err,
    );
  }
}
