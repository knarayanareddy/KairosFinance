...
function buildPrompt(verdict, result, snapshot) {
...
export async function generateNarration(verdict, result, snapshot) {
    try {
        ...
        if (!block || block.type !== 'text')
            throw new Error('No text block in narration response');
        const text = block.text.trim();
        if (!text)
            throw new Error('Empty narration returned');
        return text;
    }
    catch (err) {
